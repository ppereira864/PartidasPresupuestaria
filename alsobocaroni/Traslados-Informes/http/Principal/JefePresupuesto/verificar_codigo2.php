...
function listar_detalle($valor, $valor2, $valor3, $valor4, $valor5, $valor6){
	...
	$VerificandoPct = "SELECT * FROM calculos_pct WHERE (cod_part='$valor' AND cod_gen='$valor2' AND cod_esp='$valor3' AND cod_sub='$valor4')";

	$ejecutar_VerificandoPct = $conexion->query($VerificandoPct);

	if ( !($row = $ejecutar_VerificandoPct->fetch_assoc()) ) {
		...
		if ($row = $ejecutar_consulta->fetch_assoc()) {
			...
			<table>
		 	...
		 	<tr>
		 		...
		 		<td>
		 			<input type='text' name='monto1' id='monto1' min='1' max='1000000000000' onmouseover='verificar_monto1();'>
		 			<botton class='btn-veriicar' onmouseover='verificar_monto1();' >></botton>
		 		</td>
		...
		echo "
			<strong style='color:#C82828;'>Error Partida Cedente</strong>
		";
	}
